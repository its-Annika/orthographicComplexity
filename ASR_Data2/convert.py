...
try:
	os.mkdir(langCode+"ASR_FilesLarge")
except:
	pass

# make new inner directory
os.mkdir(langCode+"ASR_FilesLarge"+"/"+re.sub(".txt","", file))

#write the new asr folders
#one with the id and path
#one with the id and transcript
with open(langCode+"ASR_FilesLarge"+"/"+re.sub(".txt","", file)+  "/audio_paths", "w+") as af, open(langCode+"ASR_FilesLarge"+"/"+re.sub(".txt","", file)+ "/text", "w+") as tf:
...
dst_path = os.path.join(os.getcwd(), langCode+"ASR_FilesLarge"+"/"+re.sub(".txt","", file), file)
