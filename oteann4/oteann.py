...
def add_samples(config, language, task):
    ...
    for index, line in df.iterrows():
        ...
        if n < n_test:
            df_test = pd.concat([df_test, pd.DataFrame(sample, index=[0])], ignore_index = True)
            # df_test = df_test.append(sample, ignore_index = True)
        else:
            df_train = pd.concat([df_train, pd.DataFrame(sample, index=[0])], ignore_index = True)
